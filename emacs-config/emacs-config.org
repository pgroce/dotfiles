#+STARTUP: indent
#+PROPERTY: header-args :tangle build/emacs-config.el
...
I do several configurations contingent on the presence of certain commands; if the command exists, I tweak something, and if it doesn't, I usually want to mention it to the user. (You know, me.)
...
Anyway, time to configure =unicode-fonts=! The following list of Unicode blocks has been pared down to (hopefully) improve performance while removing glyphs I'll likely never need.

#+name: unicode-fonts-setup-tiny
#+begin_src emacs-lisp
...
#+name: unicode-fonts-setup-small
#+begin_src emacs-lisp :tangle no
  (use-package unicode-fonts
    :ensure t
    :config (unicode-fonts-setup)
...
The more comprehensive version is here, if any of them need to be dredged back up.

#+name: unicode-fonts-setup-big
#+begin_src emacs-lisp :tangle no
  (use-package unicode-fonts
    :ensure t
    :config (unicode-fonts-setup)
...
These are mainly just tweaks to existing org-mode parameters. One exception is =pg-org-interactive-todo=, which is bound to =C-c C-t=. (Or just =t= in =org-agenda-mode=.)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun pg-org-agenda-keywords-for-item ()
    "Get keywords for an item in the agenda buffer, based on the
  keywords in the item's original buffer. If not in the agenda
  buffer, return nil. Code adapted from `org-agenda-switch-to'."
        ...
        (let ((todo-keyword
               (completing-read "Status: "
                                (or org-todo-keywords-1
                                    (pg-org-agenda-keywords-for-item)))))
...
#+BEGIN_SRC emacs-lisp
  (use-package org-agenda
    :bind (("C-c a" . org-agenda)
           ...
           ("C-n" . next-line)
           ("C-p" . previous-line)
           ("t" . pg-org-interactive-todo)
           ("C-c C-t" . pg-org-interactive-todo))
...
#+begin_src emacs-lisp
  (use-package org-ref :ensure t :after (org pdf-tools)
...
The Python community has chosen to solve this in a predictable fashion, by installing more versions of Python. But they'll get it right this time! They swear!
    ...
    (progn
      ...
      (if (load "flycheck" t t)
          (progn
            ...
            ;; TODO: This is cargo culted. I like pylint, it seems to
            ;; work. Not sure why they're being disabled here though.
...
My priorities used to be maximizing the number of available modifiers, Space Cadet style. It turns out there's a lot to be said for symmetry on the left and right hand, though. I've become attached to =super=, though, so keep it on =option=, which is functionally =AltGr=.
...
#+BEGIN_SRC emacs-lisp :tangle build/darwin.el
  ...
  ;; ns-right-command-modifier should, by default, be 'left, inheriting
  ;; ns-command-modifier
