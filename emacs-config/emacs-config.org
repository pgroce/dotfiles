...
Because of this, defer all Super bindings so they run after the [[Decontaminate =super= keybindings][superpocalypse]].
...
(Note that I don't have to protect these bindings from the [[Decontaminate =super= keybindings][superpocalypse]] because they're called by the terminal config itself, which can sequence things appropriately.)
...
Use [[https://github.com/pgroce/emacs-pkgs/blob/master/pg-sh.org][pg-sh]] to make using shells a little more pleasant. Defer it so the keybindings make it through the [[Decontaminate =super= keybindings][superpocalypse]].
...
#+BEGIN_SRC emacs-lisp :tangle darwin.el
  (pg-shell-path-with ("PATH" :into t)
    (pg-util-list-add-unique '("/usr/local/bin" "/usr/local/sbin") it))
#+END_SRC
