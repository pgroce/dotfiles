  ...
  (use-package projectile
    ...
    (progn
      ...
      (defun pg-projectile-shell-in-project ()
        "Start a shell associated with this project, or switch to
  the shell buffer if one already exists.

  This is similar to `ansi-term', but with the buffer naming and
  persistence semantics of `projectile-run-eshell'."
        (interactive)
        ;; Mainly ansi-term, without any of the code that mangles the
        ;; buffer name.
        (let* ((buff-name (format "*[%s] Term*" (projectile-project-name)))
               (buff (get-buffer-create buff-name))
               (shell-name (or explicit-shell-file-name
                                 (getenv "ESHELL")
                                 shell-file-name))
               (buff (term-ansi-make-term buff-name shell-name)))
          (set-buffer buff)
          (term-mode)
          (term-char-mode)
