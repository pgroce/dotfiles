...
Exactly why this works is described below.

More precisely, Eglot relies on =project-find-functions= from =project.el=. [fn:: Projectile plays nicely with this; [[https://www.unwoundstack.com/blog/eglot-finding-sub-projects.html][Unwound Stack]] has a similar article to this note about using Projectile with Eglot and subprojects.] The default value of =project-find-functions= is ='(project-try-vc)=. So Eglot will ask =project-find-functions= where the project root is, and by default, =project-root-functions= will return whatever the root of version control is.
...
The =project-find-functions= help text isn't very helpful: We are told it should return "...a project instance. The exact form of the project instance is up to each respective function; the only practical limitation is to use values that ~cl-defmethod~ can dispatch on, like a cons cell, or a list, or a CL struct." What it really means is that it should be a value corresponding to one of the backends in =project.el=, which are implemented using =cl-defmethod=. That is to say, you need to return something that plays nicely with one of =project='s existing backends, or write one yourself.

This would be a blocker for me—I'm not writing a whole project backend, that's work—but fortunately a suitable last-ditch backend already exists. The =project.el= library implements a =transient= backend for projects arbitrarily supplied by users as a path; it assumes the project root is the supplied directory, and has defined methods for finding files, directories, etc., using the file system.

***** A Caveat
:PROPERTIES:
:CUSTOM_ID: eglot-caveat
:END:

One thing to be aware of here: None of the project-based VC commands will work, because the root of your project is no longer your VC root. The right answer here really is to write a new backend. Absent that, you have to pick your poison: Do you want =project.el= to play nicely with your Python subproject, or your Git project?

You have four alternatives, as I see it:

 1. Change into your VC project root and type =M-x magit= (or whatever) there. Or use =git= from the command line. These both work fine, though they're not as frictionless as =C-x C-p e=.
 2. If you delete =.subproject=, this behavior goes away and you can use the location of =.git= as your project root again. This is another reason to make this file something you can get rid of, and not something important like =pyproject.toml=.
 3. A variation of the previous option is to set =project-find-functions= in =.dir-locals.el=. If you want this behavior consistently in some directories but not in others and you don't want to delete the canary file, this may be more convenient.
 4. You can modify =project-find-functions= to disable =pg-find-subproject= globally, possibly in a command bound to a key so you could move quickly between them. It's more work, but if you did it this way you could use files like =pyproject.toml= that should stick around. The limit of this thinking is a fully-fledged UI for manipulating =project-find-functions=, which seems like a /lot/ more work, but also a fun project if you want to learn more elisp.

I am going with Options 1 and 2 for now.
...
The ~eglot-server-programs~ variable is an enormous mess and I don't see how using ~add-to-list~ here makes it any better, but this is apprently [[https://www.gnu.org/software/emacs/manual/html_node/eglot/Setting-Up-LSP-Servers.html][the canonical way to customize servers for Eglot]].
