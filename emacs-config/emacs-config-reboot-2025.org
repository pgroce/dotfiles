...
While ~tab-bar-new-tab~ will open a new, "blank" tab, it isn't bound to anything by default. I never get much value out of a "start page," but I often find I want to do spin out into a new task from a buffer I have to hand, so =pg-new-tab-current-buffer= is my "new tab" key.
...
#+begin_src emacs-lisp
  ...
  (bind-key "C-x t >" #'pg-new-tab-current-buffer)
#+end_src
