...
This is a problem I ran into and a fix. Not one that I want to bake into my config, but I think a note about it here will be useful.

Consider a monorepo =src= with two Python projects =src/py-a= and =src/py-b=. They are both managed as independent projects with =py-a/.venv= and =py-b/.venv= virtual environments. The =.git= directory, of course, is in =src=.
...
#+begin_src emacs-lisp
  ...
  (defun pg-set-subproject-support (sym new-val)
    (delete-hook 'project-find-functions #'pg-find-subproject)
