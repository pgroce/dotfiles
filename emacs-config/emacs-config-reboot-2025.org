...
I set most of my =defcustom= variables in my config either with =setopt= or with the =:custom= tag of =use-package=, so I am tempted to set the Custom file to a temp file and ignore it.
...
As satisfying as that is, I have one use case for Customize that I find compelling: storing [[#configuration-features][configuration features]] for this config.
I therefore use the following in my own configuration file.

My policy is to use =defcustom= for anything I regard as a configuration parameter, so if you're okay with multiple sources of truth, go for it. Just consider checking the file from time to time to make sure nothing's been inadvertently been retained in there. (This happens to me frequently when I experiment with fonts.)
...
Regardless, I do /define/ custom variables, and want them grouped.

#+begin_src emacs-lisp
  (defgroup pg-config '()
    "Configuration parameters for pg-config"
    :group 'emacs)
#+end_src
...
Ironically—for me, someone who doesn't like the customization interface—this setup will probably work flawlessly  if you use Customize to set the variables. As noted in the code comments, however, nothing happens without a reboot anyway. If you want to change the defaults, it's probably best to do it here, prior to installation. I realize this is awkward and may introduce a cleaner solution at some point, but this will have to do for now.
...
#+begin_src emacs-lisp
  ...
  (defcustom pg-cf-python t
    "Config option to enable/disable extra Python support (virtualenvs,
...
#+begin_src emacs-lisp
  ...
  (require 'pg-devel-python)

  (require 'pg-org)
  (require 'pg-anki)
  (require 'pg-pandoc-config)
#+end_src
...
You can replace these (and other) font faces yourself in number of places—the platform,  host, or user files, or in =scratch.el=.
...
#+begin_src emacs-lisp
  ...
  ;; https://www.programmingfonts.org/
  (pg-set-face-attribute 'fixed-pitch "Monaspace Neon Frozen" :height .90)

  ;; https://fonts.google.com/specimen/EB+Garamond
  (pg-set-face-attribute 'variable-pitch "EB Garamond"
    :height 1.3 :weight 'normal) ;180
...
If I can't find a place for something and it's too small to put in its own module, it will probably go here.
*** GUI frame setup
...
A great explainer on ~repeat-mode~: [[https://karthinks.com/software/it-bears-repeating/][It Bears Repeating: Emacs 28 & Repeat Mode]].
...
Native Emacs navigation has gotten progressively more powerful over the last few years, with tabs (see below), persistent windows, side windows, and more. You can get a lot of info and inspo from [[https://www.masteringemacs.org/article/demystifying-emacs-window-manager][Demystifying Emacs's Window Manager]] and [[https://karthinks.com/software/emacs-window-management-almanac/][The Emacs Window Management Almanac]].

Many folks advocate using ~display-buffer-alist~ to create your One True Window Layout. This is certainly the way to go if you're looking for a particular static layout—to emulate an IDE layout and behavior you like, for example.

As a guy who grew up professionally with Vim and later Emacs, I prefer to find a window layout that works for each task, keep it around while I need it, then get rid of it. Tabs, dedicated windows, and side windows make this way of working even easier. You'll find some additional configuration for all these below, however; what I'm looking for is the ability to seamlessly shift from working on something to reconfiguring my workspace, and back to work.

Many of the keybinds under ~C-x w~ are very useful for this way of working. ~C-x w C-h TAB~ will get you a list of functions. I'm particularly fond of =toggle-window-dedicated=. Most of these commands are thematically related to those, so I put them under the =C-x w= map too.

**** Open current buffer in new tab

[[https://www.gnu.org/software/emacs/manual/html_node/emacs/Tab-Bars.html][Tabs]] are a thing now. Type =C-x t C-H= and =TAB= for a list of available commands. Additional reading:

  - [[https://mmk2410.org/2022/02/11/using-emacs-tab-bar-mode/][Using Emacs tab-bar-mode]] by Marcel Kapfer
  - [[https://systemcrafters.net/emacs-tips/tab-bar-workspaces/][Organize Your Windows with the Tab Bar Emacs 27]] from System Crafters

The default key bindings aren't too bad, especially with =repeat-mode= enabled.

While ~tab-bar-new-tab~ will open a new, "blank" tab, it isn't bound to anything by default. I never get much value out of a "start page." Starting with the buffer I'm currently working in makes a lot more sense.

#+begin_src emacs-lisp
  (defun pg-new-tab-current-buffer ()
    (interactive)
    "Create a new tab from the current buffer."
    (switch-to-buffer-other-tab (current-buffer)))

  (bind-key "C-x t T" #'pg-new-tab-current-buffer)
#+end_src

**** Pinning a window to the frame edge

#+begin_src emacs-lisp
  ;; Pinning a window to the left/right side. I don't see value in
  ;; creating top/bottom side windows at the moment, but it would be
  ;; easy enough.

  (defun pg--side-current-buffer (lrdu)
    "Convenience function to implement pg-*-side-current-buffer functions."
    (display-buffer-in-side-window (current-buffer) `((side . ,lrdu) (slot . 0))))

  (defun pg-left-side-current-buffer ()
    (interactive)
    "Display the current buffer in a left side window in slot 0. Repeated use
  of this command with different current buffers will replace the buffer
  in the side window."
    (pg--side-current-buffer 'left))

  (defun pg-right-side-current-buffer ()
    (interactive)
    "Display the current buffer in a right side window in slot 0. Repeated use
  of this command with different current buffers will replace the buffer
  in the side window."
    (pg--side-current-buffer 'right))


  (bind-key "C-x w >" #'pg-right-side-current-buffer)
  (bind-key "C-x w <" #'pg-left-side-current-buffer)
#+end_src


**** Window configuration shortcuts (including =windmove=)

The ~defvar-keymap~ macro is new in Emacs 29. There's more info, including a great external link, in [[#repeat-map][the section on ~repeat-map~ ]].

#+begin_src emacs-lisp
  ;; Use windmove to swap buffers into different windows. Make the keys
  ;; repeat with repeat-mode (which must be enabled, see below)
  ...
  ;; Use C-x O to switch to the previous window in the order, not the
  ;; next. This provides consistency with the pg-nav-repeat-map.
  (defun pg-previous-window ()
    "Select the previous window in cyclic ordering of windows."
    (interactive)
    (other-window -1))

  (bind-key "C-x O" #'pg-previous-window)
#+end_src

**** Window management repeat map

See the blogs mentioned in [[#repeat-mode][the section on Repeat mode]] for more information on ~defvar-keymap~.

#+begin_src emacs-lisp

  ;; I don't want to make C-x o/O part of this keymap, but I want that
  ;; functionality when I'm _in_ this keymap. Solution: Make proxy
  ;; functions that can be part of this keymap and leave other-window
  ;; alone.
  (defun pg-other-window () (interactive) (other-window 1))
  (defun pg-previous-window () (interactive) (other-window -1))

  ;; Make the proxies accessible via C-x w, on the desired keys
  (bind-key "C-x w \\" #'split-window-right)
  (bind-key "C-x w -"  #'split-window-below)
  (bind-key "C-x w |"  #'split-root-window-right)
  (bind-key "C-x w _"  #'split-root-window-below)

  ;; Put all these on the same repeating keymap to facilitate window
  ;; configuration
  (defvar-keymap pg-nav-repeat-map
      :repeat t
      "<left>"  #'windmove-swap-states-left
      "<right>" #'windmove-swap-states-right
      "<up>"    #'windmove-swap-states-up
      "<down>"  #'windmove-swap-states-down
      ">"       #'pg-right-side-current-buffer
      "<"       #'pg-left-side-current-buffer
      "o"       #'pg-other-window
      "O"       #'pg-previous-window
      "\\"      #'split-window-right
      "-"       #'split-window-below
      "|"       #'split-root-window-right
      "_"       #'split-root-window-below
      "d"       #'toggle-window-dedicated
      "T"       #'pg-new-tab-current-buffer)
#+end_src
...
*** Better auto-save behavior (=super-save-mode=)
#+begin_src emacs-lisp
...
*** Colorize ansi-color buffers

#+begin_src emacs-lisp
  (defun display-ansi-colors ()
    "Fontify buffer according to the ansi color codes embedded in it.

  Use this function if you redirected output from a terminal command to a
  buffer and it contains ANSI color codes."
    (interactive)
...
*** Better mode line (=doom-mode-line=)

I don't use all of Doom, but [[https://github.com/seagle0128/doom-modeline/tree/master][Doom modeline]] is great.
...
*** Filling/unfilling paragraphs

(Stolen from [[https://endlessparentheses.com/fill-and-unfill-paragraphs-with-a-single-key.html][Artur Malabarba]].)

#+begin_src emacs-lisp
  (defun pg-fill-or-unfill ()
    "Like `fill-paragraph', but unfill the paragraph if used twice."
    (interactive)
    (let ((fill-column
           (if (eq last-command 'pg-fill-or-unfill)
               (progn (setq this-command nil)
                      (point-max))
             fill-column)))
      (call-interactively #'fill-paragraph)))

  (bind-key "M-q" #'pg-fill-or-unfill)
#+end_src
...
**** MCT (Deprecated)
Emacs should make MCT the default selection UI. It isn't perfect, but it provides great value out of the box. I used it for a long time, but have decided to move on to Vertico.
...
[[https://github.com/minad/vertico][Vertico]] is a strict superset of MCT. I found MCT quite adequate, but Vertico is faster and has some desirable options.



#+begin_src emacs-lisp
  ;; Emacs minibuffer configurations.
  ;; (Mostly to play nice with Vertico)
  (use-package emacs
    :custom
    ;; Enable context menu. `vertico-multiform-mode' adds a menu in the minibuffer
    ;; to switch display modes.
    ;; (Also wanted by Embark  —pg)
    (context-menu-mode t)
  ...
  ;; (Already configured at top-level —pg)
  ;; (use-package savehist
  ;;   :init
  ;;   (savehist-mode))

  ;; Vertico wants read-extended-command-predicate to be
  ;; #'command-completion-default-include-p, which is set by corfu right
  ;; now.




  ;; (The below is modified copypasta from their website —pg)
  ;; Kool-Aid from https://www.chiply.dev/post-vompeccc#vompeccc-framework
    ...
    (
     ...
     (vertico-multiform-commands
      `((consult-imenu buffer indexed)
        ...
        (consult-line-multi buffer indexed)))
...
**** Better =delete= behavior in minibuffer file searches
...
#+begin_src emacs-lisp
  ...
  ;;; pg-completion-minibuffer.el ends here
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;;; pg-completion-selection.el ends here
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;;; pg-completion-in-buffer.el ends here
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package embark
    ...
    ;; (context-mode enabled in vertico —pg)
    ;; (context-menu-mode 1)
  ...
  ;;; pg-context-actions.el ends here
#+end_src
...
#+begin_src emacs-lisp
  (use-package dired
    :bind
    (:map dired-mode-map ("." . dired-create-empty-file))
    :config
...
#+begin_src emacs-lisp
  (use-package dirvish
    ...
    (("C-c f" . dirvish)
     ...
     ("D" . dirvish-side)                ; Open a Dirvish side window
...
#+begin_src emacs-lisp
  ;; Simplify piping output to a new buffer
  ;; Adapted from
  ;; https://christiantietze.de/posts/2025/10/redirect-output-to-buffers-from-emacs-eshell/

  (defun pg-eshell-new-buffer (&optional buf-name)
    "Create and switch to new empty buffer"
    (let ((buf-name (or buf-name "*eshell command output*")))
      (switch-to-buffer (generate-new-buffer buf-name))))


  (defalias 'to #'pg-eshell-new-buffer)
...
#+begin_src emacs-lisp
  ...
  ;;; pg-serial-terminal.el ends here
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package magit
    :ensure t
    :after (project)
    :commands (magit magit-status magit-project-status magit-init)
    :bind
    (:map project-prefix-map
          ("v" . pg-project-magit))
    :init
    ;; (This is also in magit-extras, but it doesn't seem to fire
    ;; reliably if I just let nature take its course....)
    ;;
    ;; Only modify if it hasn't already been modified.
    (when (equal project-switch-commands
...
#+begin_src emacs-lisp
  ...
  (use-package project :after (magit)
    :config
...
**** Projects, Eglot, and LSPs

This is a problem I ran into and a fix. Consider a monorepo =src= with two Python projects =src/py-a= and =src/py-b=. They are both managed as independent projects with =py-a/.venv= and =py-b/.venv= virtual environments. The =.git= directory, of course, is in =src=.

Eglot, in my experience, will find and use the proper LSP and virtualenv if =exec-path= et al. are set correctly. By default, however, it will use a project root of =src=, because that's where =git= is. We want it to use =src/py-a= (or =py-b=) because that's the logical root of our Python project. It's where =basedpyright= will output stub files, for instance, and where =uv= thinks the project root is.

***** The Fix

The fix for this (well, /a/ fix) is the following (disabled by default). It allows one to set arbitrary subprojects by creating a =.subproject= file in a directory. However, project commands are slower because it uses the =transient= project type rather than the =vc= type . The former uses the filesystem to serve project commands, the latter uses a vesion control system. (If you'll never use this outside version control, you should be fine changing ='transient= below to ='vc=.)

#+begin_src emacs-lisp :tangle no
  (defun pg-find-subproject (dir)
    "Use the nearest parent directory containing a \"subproject.org\" file as
  the project root. Used to tag subdirectories of VC repositories that
  should be regarded as separate projects.

  The file can be empty, but the .org prefix was chosen to make it a
  useful file for taking subproject notes, should that be desired."
    (when-let* ((df (locate-dominating-file dir ".subproject")))
      (cons 'transient df))) ;; could also use (list 'vc 'git df)

  (defun pg-set-subproject-support (sym new-val)
    (set sym new-val)
    (remove-hook 'project-find-functions #'pg-find-subproject)
    (when (not (eq new-val nil))
      (add-hook 'project-find-functions #'pg-find-subproject)))


  (with-eval-after-load 'project
    (defcustom pg-subproject-support nil
      "If enabled, `project-root' will be found if a .subproject file is
  present in this or a parent director, effectively creating
  subprojects. These aren't as fast as regular projects, so you should
  only enable this if the default behavior is causing problems."
      :type 'boolean
      :group 'pg-config
      :initialize #'pg-set-subproject-support
      :set #'pg-set-subproject-support))

#+end_src

I prefer to change the project root only when starting language servers. I therefore enable the (somewhat hairier) following code by default. (Credit to [[https://gist.github.com/pesterhazy/e8e445e6715f5d8bae3c62bc9db32469][this Gist]] for introducing me to =eglot-server-context=.)
...
#+begin_src emacs-lisp
  ...
  ;;
  ;;
  ;; These functions operate together to locate subproject roots only
  ;; when Eglot is doing operations on language servers---i.e., when
  ;; `eglot-lsp-context' is set and
  ;; true. `pg-directory-distance-from-ancestor' may also be of general
  ;; value.
  ;;

  ;; There are two reasonable ways to go about finding the relative
  ;; depth of the current directory from the subproject directory. One
  ;; is to re-implement a version of `locate-dominating-file', tracking
  ;; and returning the relative depth. The other, implemented here, is
  ;; to identify the enclosing project directory with
  ;; `locate-dominating-file' and determine relative depth by analyzing
  ;; the two filenames.
  ;;
  ;; This approach might do a touch more work, but it is more modular
  ;; (the depth-finding function may be useful elsewhere), and the code
  ;; maintained for this project is smaller and simpler.


  (defun pg-directory-distance-from-ancestor (dir ancestor)
    "Return the distance of DIR from its ANCESTOR. Returns 0 if they are the
  same directory, 1 if ANCESTOR is the immediate parent, etc. Returns nil
  if DIR is not a child of ANCESTOR."
    (let ((dir (file-name-as-directory
                (expand-file-name dir)))
          (ancestor (file-name-as-directory
                     (expand-file-name ancestor))))
      (when (s-starts-with? ancestor dir)
        (let ((depth 0)
              (curr-dir dir))
          (while (not (null curr-dir))
            (if (s-equals? ancestor curr-dir)
                ;; break
                (setq curr-dir nil)
              (progn
                ;; Strip directory component off curr-dir
                (setq depth (1+ depth)
                      new-dir (file-name-directory
                               (directory-file-name curr-dir)))
                (if (s-equals? new-dir curr-dir)
                    ;; We're at the top. Break
                    (setq curr-dir nil)
                  ;; the search continues with new-dir
                  (setq curr-dir new-dir)))))
          depth))))

  (defun pg-locate-with-depth (dir name-or-pred)
    "Like `locate-dominating-file', but return a list containing the
  dominating directory and the distance from DIR, as computed by
  `pg-directory-distance-from-ancestor'."
    (let* ((parent (locate-dominating-file dir name-or-pred)))
      (when (not (null parent))
        `(,parent ,(pg-directory-distance-from-ancestor dir parent)))))

  (defun pg-lsp-project-root (project-markers dir)
    "Given a list of project marker file and a directory, find the nearest
  enclosing directory (least relative depth) containing a project marker."
    (when-let* ((depths (--map (pg-locate-with-depth dir it) project-markers))
                (filtered (--filter (not (null it)) depths))
                (reduced (--reduce (if (< (cadr acc) (cadr it)) acc it) filtered)))
      (car reduced)))

  (defun pg-find-enclosing-project-for-eglot (dir)
    "Respect sub-projects for working with language servers, otherwise use
  the containing project."
    (when (and (boundp 'eglot-lsp-context) eglot-lsp-context)
      (when-let* ((subproject-dir
                   (pg-lsp-project-root pg-eglot-project-markers dir)))
        (if-let* ((vc-root (project-try-vc--search subproject-dir))
                  (vc-type (nth 1 vc-root)))
            (list 'vc vc-type subproject-dir)
          (cons 'transient subproject-dir)))))


  (defun pg-set-eglot-subproject-support (sym new-val)
    (set sym new-val)
    (remove-hook
     'project-find-functions #'pg-find-enclosing-project-for-eglot)
    (when (not (eq new-val nil))
      (add-hook
       'project-find-functions #'pg-find-enclosing-project-for-eglot)))

  (with-eval-after-load 'project
    (defcustom pg-eglot-project-markers '("pyproject.toml" ".git")
      "Project files that should be project markers when Eglot is working with
  a language server."
      :type '(repeat string)
      :group 'pg-config)

    (defcustom pg-eglot-subproject-support t
      "If set, automatically find subprojects within larger projects when
  working with Eglot language servers. Use this to start a language server
  in /project/subproj1 but use other project commands at the level of
  /project.

  Be sure to set `pg-eglot-project-markers' to a list of relevant project
  file markers. The \"nearest\" one (in terms of depth from current
  directory) will be used to mark the subproject."
      :type 'boolean
      :group 'pg-config
      :initialize #'pg-set-eglot-subproject-support
      :set #'pg-set-eglot-subproject-support))

#+end_src

***** The Explanation

It took me a while to get my head around this, so herewith are some notes and references.

The =project-find-functions= help text isn't very helpful: We are told it should return "...a project instance. The exact form of the project instance is up to each respective function; the only practical limitation is to use values that ~cl-defmethod~ can dispatch on, like a cons cell, or a list, or a CL struct." What it really means is that it should be a value corresponding to one of the backends in =project.el=, which are implemented using =cl-defmethod=. (Making the self-documenting features of Emacs less helpful.) That is to say, you need to return something that plays nicely with one of =project='s existing backends, or write one yourself. [fn:: Projectile plays nicely with this; [[https://www.unwoundstack.com/blog/eglot-finding-sub-projects.html][Unwound Stack]] has a similar article to this note about using Projectile with Eglot and subprojects.]

The default value of =project-find-functions= is ='(project-try-vc)=. So Eglot will ask =project-find-functions= where the project root is, and by default, =project-root-functions= will return whatever the root of version control is.

The =project-try-vc= function, adhering to what =project.el= expects,  returns a =cons= cell whose =car= is a symbol indicating the type of project, in this case =vc=. The =cdr= is a list whose length and elements are project-specific. =vc= requires two elements in this list:[fn:: This means, of course, that the return value of =project-try-vc= is equivalent to a three-element list.] a symbol indicating what type of (supported) version control system is used, and the path to the project root. An example: ~(vc Git "~~/src/dotfiles/")~.

The =project.el= library supports a number of version control backends—Git, Mercurial, and so on—all supported through the =vc= project type. Additionally, it provides a =transient= project type, which assumes only that the project root is a directory and uses filesystem queries to support project commands.

The most conservative approach, then, is to use the =transient= backend with the subproject directory of your choice. This works, but it's much slower than the VC backends. Surprisingly, however, you don't have to specify the root of the VC project to use the VC backends! The =project.el= libraries requirements of the VC backend are very modest, so it just doesn't come up.

You can customize this behavior, for instance to root your project at the location of a canary file like =.subproject=. (Another very reasonable alternative is =pyproject.toml=, but I decided I didn't want "Python is project root" to trump "Git is project root" as general behavior; see also [[#eglot-caveat][A Caveat]].) A complication, however, is that an element of =project-find-functions= has a special return type. On success, for instance, =project-try-vc= in the above example will return ~(vc Git "~/src/dotfiles/")~.

**** Improved compilation output (=fancy-compilation=)

More info on [[https://github.com/emacsmirror/fancy-compilation][Github]].
...
#+begin_src emacs-lisp
  ...
  ;;; pg-devel-ansible.el ends here
#+end_src

*** Python (=pg-devel-python=)
:PROPERTIES:
:header-args: :tangle build/pg-modules/pg-devel-python.el
:END:

My =autovenv= package automatically sets the appropriate virtual environment for each buffer. [[https://github.com/pythonic-emacs/pyenv-mode][Pyenv mode]] sets some additional context which is used by =python.el=, =doom-modeline=, and (I assume) others.
...
#+begin_src emacs-lisp
  (use-package autovenv
    :ensure t
    :if pg-cf-python
    :functions (autovenv-find-and-activate autovenv-window-function)
    :hook ((eshell-directory-change-hook . autovenv-find-and-activate)
           ((window-buffer-change-functions
             window-selection-change-functions) . autovenv-window-function)))

  ;; Autovenv sets VIRTUAL_ENV and adds the venv to exec-path and
  ;; PATH. Pyenv doesn't set any of those (it sets other things), but it
  ;; doesn't handle locating the virtual environment. So hook Pyenv into
  ;; Autovenv and get the best of both worlds.

  (when pg-cf-python
    (defun pg-pyenv-on-activate () (pyenv-mode-set (autovenv-get-python-version)))
    (defun pg-pyenv-on-deactivate () (pyenv-mode-unset))
    (use-package pyenv-mode
      :ensure t
      :config (pyenv-mode)
      :hook ((autovenv-post-activate-hook . pg-pyenv-on-activate)
             (autovenv-pre-deactivate-hook . pg-pyenv-on-deactivate))))
...
#+begin_src emacs-lisp
  (provide 'pg-devel-python)

  ;;; pg-devel-python.el ends here
...
#+begin_src emacs-lisp
  ...
  (defvar pg-org--beautify-headlines
    (let ((headline '(:family "Optima"))
          (headline-sm '(:family "Optima" :slant italic)))
      `((org-document-title (,@headline :height 2.0))
        ...
        (org-level-4 (,@headline-sm :height 1.2))
        (org-level-5 (,@headline-sm :height 1.1))
        (org-level-6 (,@headline-sm :height 1.1))
        (org-level-7 (,@headline-sm :height 1.1))
        (org-level-8 (,@headline-sm :height 1.1))))
...
#+begin_src emacs-lisp
  ...
  ;;; pg-anki.el ends here
#+end_src

** Pandoc (=pg-pandoc-config=)
:PROPERTIES:
:header-args: :tangle build/pg-modules/pg-pandoc-config.el
:END:

Naming this weirdly so it doesn't conflict with the library name.

In the time since I wrote this Joost Kremers appears to have written a [[https://joostkremers.github.io/pandoc-mode/][Pandoc mode]] that looks much nicer than my little library for general use. AFAICT, however, it doesn't support the functionality of ~pg-pandoc-org-subtree~ at all, and the rest of my use cases tend to be "Convert this markup file to DOCX so I can copy/paste it into an Office product," which ~pg-pandoc-current-buffer~ does quite adequately.
...
#+begin_src emacs-lisp
  ...
  (provide 'pg-pandoc-config)

  ;;; pg-pandoc-config.el ends here
#+end_src
